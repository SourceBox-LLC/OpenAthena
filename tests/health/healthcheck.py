        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get("status") == "ok":
                    ...
                    result.set_status(
                        "degraded",
                        f"API reported non-healthy status: {data.get('status')}",
                        data,
                    )
            else:
                result.set_status(
            ...
            result.duration_ms = int((end_time - start_time) * 1000)
            result.set_status(
                "unhealthy", "Failed to connect to API", {"error": "ConnectionError"}
            )
    ...
    def check_tables_availability(self) -> HealthCheckResult:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                result.set_status(
                    "unhealthy",
                    f"Tables endpoint returned status code: {response.status_code}",
                    {"status_code": response.status_code},
                )
...
def main():
    ...
    parser.add_argument(
        "--url", default="http://localhost:8000", help="URL of the OpenAthena server"
    )
    parser.add_argument("--json", action="store_true", help="Output in JSON format")
    parser.add_argument(
        "--check",
        choices=["api", "tables", "query", "all"],
        default="all",
