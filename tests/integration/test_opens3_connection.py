...
@pytest.mark.skipif(
    not os.environ.get("OPENS3_ENDPOINT") and not os.environ.get("S3_ENDPOINT"),
...
def test_opens3_connection():
    ...
    try:
        ...
        try:
            ...
            conn.sql(
                "CREATE TABLE test_countries AS SELECT 1 as id, 'USA' as country, 332 as population_millions;"
            )
            conn.sql(
                "INSERT INTO test_countries VALUES (2, 'China', 1412), (3, 'India', 1408), (4, 'Brazil', 217), (5, 'Nigeria', 218);"
            )

            # Query the table
            print(
                "Executing query: SELECT * FROM test_countries ORDER BY population_millions DESC LIMIT 3"
            )
            result = conn.sql(
                "SELECT * FROM test_countries ORDER BY population_millions DESC LIMIT 3"
            )
        ...
        print(
            f"Successfully verified query results: {len(data)} rows matched expected values"
        )
