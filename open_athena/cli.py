...
def parse_args():
    ...
    parser.add_argument(
        "--list-tables", 
        action="store_true",
        help="List all tables in the catalog"
    )
    ...
    parser.add_argument(
        "--add-table",
        action="store_true", 
        help="Add a new table to the catalog"
    )
    
    parser.add_argument(
        "--table-name",
        help="Name of the table to add"
    )
    
    parser.add_argument(
        "--bucket",
        help="S3 bucket for the table"
    )
    
    parser.add_argument(
        "--prefix",
        help="S3 prefix for the table"
    )
...
def main():
    ...
    try:
        ...
        print(f"Error connecting to OpenAthena server at {args.server}: {e}", file=sys.stderr)
        print("Make sure the server is running and the URL is correct.", file=sys.stderr)
        sys.exit(1)
    ...
    if args.add_table:
        if not all([args.table_name, args.bucket, args.prefix]):
            print("Error: --table-name, --bucket, and --prefix are required for --add-table",
                  file=sys.stderr)
            sys.exit(1)
    ...
    if query:
        ...
        # No action specified
        print("Error: No action specified. Use --help to see available options.",
              file=sys.stderr)
        sys.exit(1)
